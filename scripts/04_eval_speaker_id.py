#!/usr/bin/env python3
"""
Evaluate a trained ``SpeakerModel`` checkpoint on held-out manifest rows.

Default: ``subset=train-clean-100`` and ``split=test`` (closed-set test utterances).
Skips rows whose ``speaker_id`` is missing from ``speaker_label_map.json``.
"""
...
def main() -> int:
    p = argparse.ArgumentParser(description="Evaluate speaker-ID checkpoint on manifest split.")
    ...
    p.add_argument("--split", default="test", help="Manifest split column value (default: test).")
    ...
    p.add_argument(
        "--metrics-json",
        type=Path,
        default=Path("results/speaker_baseline/eval_test_metrics.json"),
        help="Write accuracy + counts here.",
    )
    args = p.parse_args()
    ...
    if num_classes != len(speaker_to_idx):
        print(
            f"Warning: checkpoint num_classes={num_classes} but label map has {len(speaker_to_idx)} entries.",
            file=sys.stderr,
        )
    ...
    print(f"subset={args.subset!r} split={args.split!r}  n={len(rows)}  skipped_unknown_speaker={skipped}")
    print(f"test_accuracy (closed-set) = {acc:.6f}  device={device}")
